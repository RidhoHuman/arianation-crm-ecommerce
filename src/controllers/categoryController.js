...
const getAllCategories = async (req, res, next) => {
  try {
    ...
    const mappedCategories = categories.map(c => ({
      ...c,
      name: c.categoryName,
      slug: c.categoryName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, ''),
...
const getCategoryBySlug = async (req, res, next) => {
  try {
    ...
    const category = categories.find(c => {
      const generatedSlug = c.categoryName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');
...
const createCategory = async (req, res, next) => {
  try {
    const { name, description, isActive, businessType, imageUrl, longDescription, purpose, highlights, useCases } = req.body;
    ...
    const responseData = {
      ...
      slug: newCategory.categoryName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, ''),
...
const updateCategory = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { name, description, isActive, imageUrl, longDescription, purpose, highlights, useCases } = req.body;
    ...
    if (name) {
      ...
      const duplicate = await knex('productCategory').where('categoryName', name).whereNot('id', id).first();
      if (duplicate) {
        throw new ValidationError('Kategori dengan nama ini sudah ada');
      }
    }
    ...
    if (isActive !== undefined) {
      updateData.isActive = isActive === true || isActive === 'true' || isActive === 1 || isActive === '1';
    }
    ...
    const responseData = {
      ...
      slug: updated.categoryName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, ''),
...
const deleteCategory = async (req, res, next) => {
  try {
    ...
    const productsUsingCategory = await knex('product').where('categoryId', id).count('* as count').first();
    if (productsUsingCategory.count > 0) {
      throw new ValidationError(`Tidak dapat menghapus kategori karena sedang digunakan oleh ${productsUsingCategory.count} produk.`);
    }
