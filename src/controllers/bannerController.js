...
const createBanner = async (req, res, next) => {
  try {
    const { page_location, imageUrl, title, subtitle, buttonText, buttonLink, isActive, orderIndex } = req.body;
    ...
    res.status(201).json({ success: true, data: newBanner, message: 'Banner created successfully' });
  } catch (error) {
    next(error);
  }
};
...
const updateBanner = async (req, res, next) => {
  try {
    ...
    await knex('hero_banners').where({ id }).update({
      page_location: updates.page_location !== undefined ? updates.page_location : banner.page_location,
