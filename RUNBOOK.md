...
Prisma execution policy:
- CI validation workflows use `npx prisma db push` for schema sync.
- Manual release workflow is the only path that runs `npx prisma migrate deploy`.
- Do not treat `db push` as a replacement for tracked migration deployment.
...
Note: CI jobs intentionally use `prisma db push` for test/runtime readiness. Use `prisma migrate deploy` only in controlled manual release flow after PostgreSQL migration history is reconciled.
